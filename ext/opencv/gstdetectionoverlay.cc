    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    ...
    case PROP_LABELS:
      ...
      detection_overlay->labels_list =
          g_strsplit (g_value_get_string (value), ";", 0);
      detection_overlay->num_labels =
          g_strv_length (detection_overlay->labels_list);
    ...
    GstVideoFrame * frame)
{
  ...
  for (i = 0; i < detect_meta->num_boxes; ++i) {
    ...
    cv::putText (cv_mat, str, cv::Point (box.x, box.y - 5),
        cv::FONT_HERSHEY_PLAIN, detection_overlay->font_scale,
        colors[box.label % N_C], detection_overlay->box_thickness);
